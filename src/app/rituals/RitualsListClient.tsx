        ...
        <div className={styles.container}>
            <div className="glass-panel" style={{ padding: '1.5rem', marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <h1 className={styles.header} data-testid="rituals-page-title" style={{
                        fontSize: '2rem',
            ...
            {isCreating && (
                <div className="glass-panel" style={{ padding: '2rem', marginBottom: '2rem', borderTop: '4px solid hsl(var(--accent))' }}>
                    <h2 style={{ marginBottom: '1rem', color: 'white' }}>Programar Nuevo Ritual</h2>
                    <form action={createRitual} className={styles.formRow} style={{ alignItems: 'flex-end', display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', flex: 1 }}>
                            <label style={{ fontSize: '0.9rem', color: 'rgba(255, 255, 255, 0.8)' }}>Fecha de Sesión</label>
                            <input name="date" type="date" required style={{ width: '100%' }} />
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', flex: 2 }}>
                            <label style={{ fontSize: '0.9rem', color: 'rgba(255, 255, 255, 0.8)' }}>Contexto / Descripción</label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '2rem' }}>
                {rituals.length === 0 && (
                    <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '4rem', color: 'rgba(255, 255, 255, 0.8)' }}>
                    ...
                    <Link key={ritual.id} href={`/rituals/${ritual.id}`} style={{ textDecoration: 'none' }}>
                        <div className="glass-panel" style={{
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <h3 style={{ fontSize: '1.25rem', fontWeight: 600, color: 'white' }}>{ritual.name}</h3>
                                <span style={{
                                    fontSize: '0.8rem',
                            ...
                            <p style={{ color: 'rgba(255, 255, 255, 0.8)', fontSize: '0.9rem', flex: 1 }}>
